...
interface TypingAreaProps {
  targetText: string;
  onStatsUpdate: (wpm: number, accuracy: number, combo: number, progress: number) => void;
  onAttackTrigger: (power: number) => void;
}

export const TypingArea: React.FC<TypingAreaProps> = ({ targetText, onStatsUpdate, onAttackTrigger }) => {
  ...
  const renderedChars = targetText.split('').map((char, index) => {
    ...
    return (
      <span key={index} style={{ color, textDecoration: index === typedIndex ? 'underline' : 'none', fontWeight: index === typedIndex ? 'bold' : 'normal' }}>
        {char}
      </span>
    );
  });
