import type { ThemeRenderer } from '../ThemeInterface';

export class FightingRenderer implements ThemeRenderer {
  private ctx: CanvasRenderingContext2D | null = null;
  private width = 0;
  private height = 0;
  private attackParticles: { x: number; y: number; vx: number; vy: number; alpha: number }[] = [];
  ...
  onAttack(sourceId: string, _targetId: string, _power: number): void {
  ...
  onDamage(_targetId: string, _amount: number): void {
    // Trigger hit effect
  }
